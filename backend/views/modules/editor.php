	...
	*/
	...
	function checkQuery() {
		...
		return	isset($app->request->query["module_title"]) &&
				...
				isset($app->request->query["module_settings"]) && is_array($app->request->query["module_settings"]) &&
	...
	$app->vars["module_id"]		= "";
	$app->vars["module_settings"] = array();
	$app->vars["module_pos"]	= "";
	...
	$app->vars["module_class_settings"] = $moduleinfo["settings"];
	...
	if (isset($app->request->query["edit_id"])) {
		...
		if (checkQuery()) {
			...
			$settings = array();
			foreach ($app->vars["module_class_settings"] as $setting)
				$settings[$setting["name"]] = $app->request->query["module_settings"][$setting["name"]];
		...
		$app->vars["module_settings"] = $module->settings;
			...
			<div class="row">
				<form method="post" class="d-flex">
					<div class="col col-lg-8 me-lg-2">
						...
						<?php
							foreach ($app->vars["module_class_settings"] as $setting) {
								echo "<label for=\"module_settings[{$setting["name"]}]\">{$setting["label"]}:</label>";
								switch ($setting["type"]) {
									case "article":
										RenderHelper::renderArticlePicker("module_settings[{$setting["name"]}]", $app->vars["module_settings"][$setting["name"]] ?? null);
										break;
									case "category":
										RenderHelper::renderCategoryPicker("module_settings[{$setting["name"]}]", $app->vars["module_settings"][$setting["name"]] ?? null);
										break;
									case "menu":
										RenderHelper::renderMenuPicker("module_settings[{$setting["name"]}]", $app->vars["module_settings"][$setting["name"]] ?? null);
										break;
									case "user":
										RenderHelper::renderUserPicker("module_settings[{$setting["name"]}]", $app->vars["module_settings"][$setting["name"]] ?? null);
										break;
									case "usergroup":
										RenderHelper::renderUserGroupPicker("module_settings[{$setting["name"]}]", $app->vars["module_settings"][$setting["name"]] ?? null);
										break;
									case "boolean":
										echo "<br /><div class=\"form-check form-check-inline\">";
										echo "<input type=\"radio\" class=\"form-check-input\" name=\"module_settings[{$setting["name"]}]\" id=\"setting_{$setting["name"]}_true\" value=\"1\"";
										if ($app->vars["module_settings"][$setting["name"]] ?? false) echo " checked";
										echo ">";
										echo "<label class=\"form-check-label\" for=\"setting_{$setting["name"]}_true\">Yes</label>";
										echo "</div>";
										echo "<div class=\"form-check form-check-inline\">";
										echo "<input type=\"radio\" class=\"form-check-input\" name=\"module_settings[{$setting["name"]}]\" id=\"setting_{$setting["name"]}_false\" value=\"0\"";
										if (!($app->vars["module_settings"][$setting["name"]] ?? false)) echo " checked";
										echo ">";
										echo "<label class=\"form-check-label\" for=\"setting_{$setting["name"]}_false\">No</label>";
										echo "</div><br />";
										break;
									case "number":
										echo "<input type=\"number\" class=\"form-control\" name=\"module_settings[{$setting["name"]}]\" value=\"" . ($app->vars["module_settings"][$setting["name"]] ?? 0) . "\" required />";
										break;
									case "longtext":
									case "editor":
										echo "<textarea class=\"form-control\" name=\"module_settings[{$setting["name"]}]\" style=\"height: 300px; font-family: monospace;\" required onkeydown=\"if(event.keyCode===9){var v=this.value,s=this.selectionStart,e=this.selectionEnd;this.value=v.substring(0, s)+'\\t'+v.substring(e);this.selectionStart=this.selectionEnd=s+1;return false;}\">" . htmlentities($app->vars["module_settings"][$setting["name"]] ?? "") . "</textarea>";
										break;
									case "string":
									default:
										echo "<input type=\"text\" class=\"form-control\" name=\"module_settings[{$setting["name"]}]\" value=\"" . ($app->vars["module_settings"][$setting["name"]] ?? "") . "\" required />";
										break;
								}
							}
						?>
