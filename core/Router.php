<?php
	...
	*/
	...
	class Router {
		public static function getSlug(bool $backend = null) : string {
			$slug = substr($_SERVER["REQUEST_URI"], strlen(Config::WEBROOT . ($backend ? "/backend" : "")) + 1);
			$slug = explode("?", $slug)[0] ?? "";
			if (substr($slug, -1) == "/") $slug = substr($slug, 0, -1);
			if (!strlen($slug)) return ($backend ? "dashboard" : "index");
			return $slug;
		}

		public static function routeRequest(bool $backend = null) {
			global $app;

			$app->events->trigger("router.route");

			$slug = Router::getSlug($backend);
			if ($backend) {
